...
# 事务的四大特性ACID
原子性(Atomicity)：不可分割，要么成功，要么失败      # 一个从失败中恢复到正常状态的方法，可以用于脚本、一次性任务、实验性任务
一致性(Consistency)：事务完成，则数据状态一致       # 在异常状态下仍能保持一致性的方法
隔离性(Isolation)：隔离机制，保证事务操作独立       # 多任务并行下，防止彼此干扰，一般需要配合锁来控制
持久性(Durability)：提交回滚，对数据永久改变        # 事务处理完成后，对数据的修改就是永久的，即便系统故障也不会丢失。事务一旦提交就无法回滚

事务的出现了是为了应用层服务的，而不是数据库系统本身的需要 。
```
