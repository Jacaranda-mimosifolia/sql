...
注意：核心思想就是动态表建立外键，连接静态表；通过两张表的某种联系获取结果；
```

### 1. 自然连接：自然连接 NATURAL JOIN [自然连接]
```
# 返回两表间字段名相同条件下值也相同的数据
# 如：
...
### 4. 联合查询：将多次查询的结果合并，结果与全外连接相似 UNION ALL
```
# 注意：分号写在末尾；union all查询所有，union去除查询；字段数量与字段类型必须一致；
select * from 表1
union [all]
select * from 表2;
...
### 6. exists
```
# 与子查询返回无关，只与子查询的where有关，若where为true，则返回父表符合where条件的数据
select * from student
